    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card card-primary">
                <div class="card-header">{{ __('EDIT USER PRIVILEGE') }}</div>
                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{route('manage.roles.update', ['role' => $role->id])}}" id="quickForm">
                        @method('PATCH')
                        @csrf
                        <div class="card-body">
                            <div class="form-group">
                                <label for="name">User Privilege Name <span class="red">*</span></label>
                                <input type="text" name="name" class="form-control" id="name" placeholder="Enter Privileg Name" value="{{old('name', $role->name)}}" style="width: 100%;" />
                                @error('name')
                            ...
                            <div class="form-group">
                                <label for="previleg">Is Deleted</label>
                                <input type="checkbox" name="deleted_at" @if (!is_null($role->deleted_at))
                                    checked="checked"
                                @endif />
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group mb-0">
                                ...
                                <select name="used_for" id="used_for" class="form-control">
                                    <option value="frontend" @if ($role->used_for == 'frontend')
                                        selected
                                    @endif>Backend</option>
                                    <option value="backend" @if ($role->used_for == 'backend')
                                        selected
                                    @endif>Frontend</option>
                                </select>
                                @error('used_for')
                                <span class="text-danger">{{$message}}</span>
                                @enderror
                            ...
                            <div class="row mt-4">
                                @php
                                    $range = $role->permissions->pluck('id')->all();
                                @endphp
                                ...
                                <div class="col-md-3">
                                    ...
                                    @if ($group_route)
                                    <ol>
                                        @foreach ($group_route as $route)
                                        <li>
                                            <input type="checkbox" name="other_permissions[]" value="{{$route->permission->id}}" @if (in_array($route->permission->id, $range)) checked @endif /> {{$route->permission->name}} {{$route->permission->id}}
                                        </li>
                                        @endforeach
                                    </ol>
                                    @endif
                        ...
                        <div class="modal fade" id="modal-default">
                            ...
                            <div class="modal-content bg-default">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div id="tree">
                                        <x-admin-menu-tree-checkbox :menus="$menus" :role="explode(',', $role->range)" />
                                    </div>
                                </div>
